...
const modules = [
  ...
  },
  {
    ...
    description: "Consultez, annulez ou remboursez les ventes passées.",
    icon: <ScrollText className="h-6 w-6 sm:h-7 sm:w-7 text-violet-600" />,
    href: "/quinc/commandes",
    ...
    CashierDashboardService.getOverview({ userId, shopId })
      ...
      .catch((err: unknown) => {
        if (!cancelled) {
          ...
          setLoading(false);
          console.error("[Dashboard Quinc]", err);
        }
      });
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 max-w-7xl mx-auto pb-24 md:pb-12 px-2 sm:px-0">
        ...
        {error && (
          <div className="px-5 py-3.5 rounded-2xl border bg-rose-50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-800/40 text-rose-600 text-sm font-bold">
            {error}
          </div>
        )}
              ...
              <>
                ...
                <div className="flex items-end gap-1 border-b border-zinc-100 dark:border-zinc-800 pb-1" style={{ height: "80px" }}>
                  {(overview?.timeline ?? []).map((t) => {
                    ...
                    return (
                      <div key={t.hour} className="group flex flex-col items-center flex-1 min-w-0 justify-end h-full">
                        <span className="text-[8px] font-black text-amber-500 mb-0.5 opacity-0 group-hover:opacity-100 transition-opacity leading-none">
                          {fmt(t.revenue)}
                        </span>
                        <div
                          title={`${t.hour} · ${fmt(t.revenue)} XOF · ${t.transactionCount} vente${t.transactionCount > 1 ? "s" : ""}`}
                          className={`w-full rounded-t transition-all duration-300 cursor-default group-hover:opacity-90 ${isMax ? "bg-amber-500" : "bg-amber-500/30 group-hover:bg-amber-500/60"}`}
                ...
                return (
                  <div key={sale.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={`text-sm font-black ${isPaid ? "text-amber-500" : "text-red-500 line-through"}`}>
              ...
              <Link href={mod.href} key={idx} className="w-full">
                ...
                >
                  <div className="flex sm:w-full items-center justify-between mb-2 sm:mb-4">
                    ...
                    <div className="hidden sm:flex h-9 w-9 items-center justify-center rounded-full bg-zinc-50 dark:bg-zinc-800 group-hover:bg-amber-500 group-hover:text-white transition-all duration-300">
