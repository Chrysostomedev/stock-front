...
const CATEGORIES: { value: ExpenseCategory; label: string }[] = [
  ...
  { value: ExpenseCategory.PURCHASE_ORDER, label: "Achat fournisseur" },
  { value: ExpenseCategory.STOCK_INITIAL, label: "Stock initial" },
...
export default function SuperDepensesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (error?.response?.status === 403) {
        showToast("Cette dépense est générée automatiquement et ne peut pas être supprimée.", "error");
      } else {
  ...
  const columns: { header: string; accessor: keyof Expense | ((item: Expense) => React.ReactNode); className?: string }[] = [
    ...
    },
    {
      ...
      accessor: (e: Expense) => (
        e.isAutoGenerated ? (
          <div className="p-2 text-zinc-300 cursor-not-allowed" title="Dépense automatique — non modifiable">
