...
export default function AdminProduitsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const prodList =
        prodRes.data && Array.isArray(prodRes.data) ? prodRes.data : [];
      const catList =
        catRes.data && Array.isArray(catRes.data) ? catRes.data : [];
      const shopList = Array.isArray(shopRes) ? shopRes : shopRes.data || [];
      const unitList =
        unitRes.data && Array.isArray(unitRes.data) ? unitRes.data : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.barcode && !formData.sku) {
      showToast(
        "Le code-barres ou la référence SKU est obligatoire pour le suivi",
        "error",
      );
      return;
    }
  ...
  const columns: {
    ...
    },
    {
      ...
      accessor: (item: Product) => (
        <div className="flex items-center gap-2">
          <span
            className={`font-black ${item.stockQty <= item.minStockQty ? "text-red-500" : "text-emerald-500"}`}
          >
            {item.stockQty}
          </span>
  ...
  return (
    <AppLayout
      ...
      subtitle="Produits, prix et inventaire global"
    >
      ...
      <Modal
        ...
        title={
          selectedProduct ? "Modifier le produit" : "Nouveau produit catalogue"
        }
        size="lg"
      >
        <form
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Référence SKU </label>
                <input 
                  type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                  Boutique Assignée <span className="text-red-500">*</span>
                </label>
                <select
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      minStockQty: parseFloat(e.target.value),
                    })
                  }
                  className="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-bold outline-none focus:border-primary transition-all"
                />
