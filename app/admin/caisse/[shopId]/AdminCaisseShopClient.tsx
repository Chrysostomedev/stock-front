...
export default function AdminCaisseShopClient() {
  ...
  const loadStaticData = async () => {
    ...
    try {
      let catRes;
      try {
        catRes = await CategoryService.getByShop(shopId, { limit: 100 });
      } catch {
        catRes = await CategoryService.getByShop(shopId);
      }
  ...
  const handleCloseSession = async () => {
    ...
    try {
      await CashSessionService.close(cashSession.id, {
        ...
        notes: `Session fermée par ${user?.name} (admin)`,
      });
      setCashSession(null);
  ...
  return (
    <AppLayout
      title={`Caisse — ${currentShop?.name || "Boutique"}`}
      subtitle={`Point de vente • Accès administrateur`}
      backUrl="/admin/boutiques"
    >
      <div style={{
      ...
      <div className="pos-root">
        {!cashSession ? (
          <div className="pos-session-banner pos-session-closed-banner">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button className="pos-session-btn" onClick={handleOpenSession} disabled={isOpeningSession}>
        ...
        <div className="pos-layout">
          <aside className="pos-sidebar">
            <div className="pos-sidebar-logo">
              <ShoppingBag size={18} />
              <div>GestShop<div className="pos-sidebar-shop">{currentShop?.name || "Boutique"}</div></div>
            </div>
            <div className="pos-cats-list">
              <button className={`pos-cat-btn ${!selectedCategory ? "active" : ""}`} onClick={() => setSelectedCategory(null)}>
                <LayoutGrid size={14} />Tous<span className="pos-cat-count">{totalProducts}</span>
              </button>
              {categories.map((cat) => (
                <button key={cat.id} className={`pos-cat-btn ${selectedCategory === cat.id ? "active" : ""}`} onClick={() => setSelectedCategory(cat.id)}>
          ...
          <div className="pos-catalog">
            <div className="pos-mobile-header">
              ...
              <div className="pos-mobile-cats">
                ...
                {categories.map((cat) => (
                  <button key={cat.id} className={`pos-mob-cat ${selectedCategory === cat.id ? "active" : ""}`} onClick={() => setSelectedCategory(cat.id)}>{cat.name}</button>
                ))}
              </div>
            </div>
          ...
          <div className="pos-cart" style={{ transform: mobileCartOpen ? "translateY(0)" : "translateY(100%)", transition: "transform .3s cubic-bezier(.32,.72,0,1)" }}>
            <div className="pos-cart-head">
              <div className="pos-cart-title">
                <ShoppingCart size={14} />Panier<span className="pos-cart-badge">{totalItems}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {pendingCarts.length > 0 && (
                  <button onClick={() => setShowPendingModal(true)} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, fontWeight: 700, color: "#7B93C8", background: "rgba(37,99,235,.12)", padding: "4px 8px", borderRadius: 8, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: ".06em" }}>
                    <Clock size={11} />{pendingCarts.length} en attente
                  </button>
                )}
                {cart.length > 0 && (
                  <button onClick={handlePutOnHold} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, fontWeight: 700, color: "#7B93C8", background: "rgba(37,99,235,.12)", padding: "4px 8px", borderRadius: 8, border: "none", cursor: "pointer", textTransform: "uppercase", letterSpacing: ".06em" }}>
                    <Pause size={11} />Attente
                  </button>
                )}
                <button className="pos-cart-clear-btn" onClick={() => setCart([])}>Vider</button>
                {cart.length > 0 && (
                  <button onClick={handleCheckout} disabled={isProcessing} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 800, color: "#fff", background: isProcessing ? "#6B7280" : "#16A34A", padding: "5px 11px", borderRadius: 8, border: "none", cursor: isProcessing ? "not-allowed" : "pointer", textTransform: "uppercase", letterSpacing: ".05em", flexShrink: 0 }}>
                    <CheckCircle2 size={12} />{isProcessing ? "…" : fmt(total)}
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="pos-cust-wrap">
              ...
              {selectedCustomer ? (
                <div className="pos-cust-selected">
                  <div className="pos-cust-name"><User size={13} />{selectedCustomer.name}</div>
                  <button className="pos-cust-clear" onClick={() => setSelectedCustomer(null)}><X size={14} /></button>
                </div>
              ) : (
            ...
            <div className="pos-cart-items">
              {cart.length === 0 ? (
                <div className="pos-cart-empty"><ShoppingCart size={36} /><p>Panier vide</p></div>
              ) : (
                cart.map((item) => (
              ...
              {paymentMethod === "CASH" ? (
                <div className="pos-cash-wrap">
                  ...
                  {received > 0 && cart.length > 0 && (
                    <div className="pos-change-row"><span className="pos-change-label">Monnaie à rendre</span><span className="pos-change-val">{fmt(change)} XOF</span></div>
                  )}
                </div>
              ) : (
                <div className="pos-mobile-ops">
                  {(["WAVE", "ORANGE", "MTN"] as const).map((op) => (
                    <button key={op} className={`pos-mobile-op ${mobileProvider === op ? "active" : ""}`} onClick={() => setMobileProvider(op)}>{op}</button>
                  ))}
                </div>
              )}
        ...
        {mobileCartOpen && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 199, backdropFilter: "blur(2px)" }} onClick={() => setMobileCartOpen(false)} />
        )}

        <div style={{ position: "fixed", top: "-9999px", left: "-9999px", width: "80mm", pointerEvents: "none", overflow: "hidden" }}>
          <TicketReceipt ref={componentRef} shop={currentShop} user={user} items={saleCartSnapshot} total={total} paymentMethod={paymentMethod} amountReceived={saleReceivedSnapshot} change={saleChangeSnapshot} saleId={lastSaleId} />
        </div>
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 28, maxWidth: 340, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,.25)", display: "flex", flexDirection: "column", gap: 20, fontFamily: "inherit" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { handlePrint(); resetAfterSale(); }} style={{ flex: 1, padding: "12px 0", background: "#2563EB", border: "none", borderRadius: 12, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer", textTransform: "uppercase", letterSpacing: ".06em", fontFamily: "inherit" }}>Oui, imprimer</button>
            </div>
          </div>
        </div>
      )}
